...
{
  int l=song->get_pattern(current_pattern)->get_length()/4;
  if(selection_end_y-selection_begin_y==l-1 && selection_begin_x==cursor_x && selection_begin_y == cursor_y)
    l*=2;

  selection_begin_x = selection_end_x = cursor_x;
  selection_begin_y = cursor_y;
  selection_end_y	= cursor_y + (l-1);
  selection_active= true;
  normalize_selection();
}
