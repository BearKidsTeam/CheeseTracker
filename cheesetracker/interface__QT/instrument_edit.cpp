...
void Instrument_Edit::load_instrument() {
    ...
    QString fn = QFileDialog::getOpenFileName(nullptr, "(load instrument)", QString(), "Instrument Files (*.ci *.CI *.ii *.II *.xi *.XI)" );
	if (fn.isEmpty())
		return; //nothing


	file_manager->load_instrument( fn.toStdString().c_str() , selected_instrument );
...
void Instrument_Edit::save_instrument() {
	...
	QString s = QFileDialog::getSaveFileName(
                nullptr, "Save (Choose a File Name)",
                QString(), "Instrument Formats (*.ci *.CI *.ii *.II)");
	...
	if (f) { //file exists
		fclose(f);
        if ( QMessageBox::warning( nullptr, "Question:","File exists! Overwrite?",
                                   QMessageBox::No | QMessageBox::Yes, QMessageBox::No) ) {
			return ;
		}
	}

        if ( file_manager->save_instrument( s.toLatin1(), selected_instrument ) ) {

        QMessageBox::warning( nullptr, "Oops!","Error saving instrument!", QMessageBox::Ok);
		return ;
	}
...
void Instrument_Edit::item_selected_cbk(QTreeWidgetItem *p_item) {
...
void Instrument_Edit::item_renamed_cbk( QTreeWidgetItem * p_item, int col ) {
    ...
    if (item==nullptr) {
        ...
        string text=item->text(col).toStdString();
                if (text.length()>25)
...
Instrument_Edit::Instrument_Edit(QWidget *p_parent) : QHBoxLayout(p_parent)  {
	...
	QVBoxLayout * instr_props_aux_vbox = new QVBoxLayout(this);

	instrument_list_group = new QGroupBox ( 1, Qt::Vertical,"Instrument List",instr_props_aux_vbox);

	selected_instrument=0;
	/* listview */
    instrument_list = new QTreeWidget(instrument_list_group);
...
//	instrument_list->setDefaultRenameAction(QListView::Accept);
    ...
    instrument_list->setHeaderLabel("#");
    instrument_list->setHeaderLabel("Name");
	...
	QHBoxLayout *aux_layer_hbox = new QHBoxLayout(instr_props_aux_vbox);
	...
	instrument_general_hbox = new QHBoxLayout(instrument_props_notebook);
	...
	instrument_volume_vbox = new QVBoxLayout(instrument_props_notebook);
	...
	instrument_pan_vbox = 	new QVBoxLayout(instrument_props_notebook);
	...
	instrument_pitchfilter_vbox = new QVBoxLayout(instrument_props_notebook);
	...
	nna.vbox = new QVBoxLayout(instrument_general_hbox);
	...
	nna.hbox = new QHBoxLayout(nna.vbox);
	...
	volume.grid = new QGridLayout(2,volume.group);
	...
	panning.grid = new QGridLayout(2,panning.group);


	QHBoxLayout *auxhv = new QHBoxLayout(panning.grid);
	...
	pitch_filter.grid = new QGridLayout(2,pitch_filter.group);
