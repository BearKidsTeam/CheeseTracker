...
void Sample_Edit::load_sample() {
    ...
    file_manager->load_sample( fn.toLatin1() , selected_sample );
...
void Sample_Edit::save_sample() {
	...
	if (f) { //file exists
        ...
        if ( file_manager->save_sample( s.toLatin1() , selected_sample ) ) {
...
Sample_Edit::Sample_Edit(QWidget *p_parent) : QHBoxLayout(p_parent)  {

	sample_list_group = new QGroupBox ( 1, Qt::Vertical,"Sample List",this);

	sample_props_vbox = new QVBoxLayout(this);
	...
	sample_vars_hbox = new QHBoxLayout(sample_editor->get_tabs());
