...
void test_sample_data()
{
	...
	// Check if dup_loop works.

	test.set_size(sizeof(in_buffer)/sizeof(sample_int_t));
	test.seek(0);
	test.set_num_channels(1);
	test.put_sample_array(in_buffer, sizeof(in_buffer)/sizeof(sample_int_t));
	test.set_loop_enabled(true);
	test.set_loop_begin(0);
	test.set_loop_end(4);
	test.dup_loop(3);
	test.seek(0);
	test.get_sample_array(out_buffer, 17);

	// Expected contents of out_buffer: {0, 1, 2, 3, 0, 1, 2, 3 ...}

	for(sample_int_t ix=0; ix<16; ix++) {
		if(out_buffer[ix] != ix % 4) {
			
			E.set_error_pfx("dup_loop");
			E.eprintf("%s:%i: Expected %d got %d", __FILE__, __LINE__, ix % 4, out_buffer[ix]);
			throw E;
		
		}
	}
	// Expected: out_buffer[16] == 16
	if(out_buffer[16] != 16) {
		E.set_error_pfx("dup_loop");
		E.eprintf("%s:%i: Expected %d got %d", __FILE__, __LINE__, 16, out_buffer[16]);
		throw E;
	}
}
